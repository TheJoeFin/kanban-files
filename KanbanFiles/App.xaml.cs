...
public partial class App : Application
{
    ...
    private void RestoreWindowState()
    {
        ...
        try
        {
            ...
            bool isMaximized = settings.Values.TryGetValue(IS_MAXIMIZED_KEY, out object? maxValue) && (bool)maxValue;
            ...
            // Restore size
            int width = settings.Values.TryGetValue(WINDOW_WIDTH_KEY, out object? widthValue) ? (int)widthValue : DEFAULT_WIDTH;
            int height = settings.Values.TryGetValue(WINDOW_HEIGHT_KEY, out object? heightValue) ? (int)heightValue : DEFAULT_HEIGHT;
            ...
            // Restore position
            if (settings.Values.TryGetValue(WINDOW_X_KEY, out object? xValue) &&
                settings.Values.TryGetValue(WINDOW_Y_KEY, out object? yValue))
            {
                int x = (int)xValue;
                int y = (int)yValue;
